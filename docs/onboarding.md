...
oak is configured declaratively from an `oak.yaml` spec. Secrets stay in env; the spec references env vars by name.

```yaml
# oak.yaml
version: 1
workspace:    { kind: e2b,   api_key_env: E2B_API_KEY }
state_store:  { kind: redis, url_env: REDIS_URL }
lease:        { kind: redis, url_env: REDIS_URL, ttl_seconds: 30 }
tracing:      { kind: staso, api_key_env: STASO_API_KEY }
framework:    { kind: openhands }
```
...
async def handle_turn(session_id: str, user_text: str):
    async with oak.session.openhands.attach(                       # [OAK] orchestrates
        ...
        result = await session.run_turn(user_text)                  # [OAK wrapper drives the framework]
...
For advanced users / library integrators who want to construct backends in Python, the imperative `oak.session.configure(state_store=..., trace=..., lease=...)` API stays available and does the same thing under the hood.

The agent loop is the framework's (OpenHands here). oak owns the runtime mechanism around it: sandbox abstraction, resumable sessions, cross-process ownership.

Bringing the infra (Redis, Postgres, sandbox provider) is your call - local containers, k8s, managed cloud, whatever fits.
